    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bolder mb-1" style="color: #0f172a; letter-spacing: -0.5px;">
                <i class="bi bi-pencil-square text-primary me-2"></i>เขียนบทความใหม่
            </h2>
            <p class="text-muted mb-0">สร้างสรรค์เนื้อหาใหม่และเผยแพร่สู่ผู้ใช้งานของคุณ</p>
        </div>
        ...
        <form action="/author/insert" method="POST">
            ...
            <div class="mb-4">
                <label for="title" class="form-label fw-bold text-dark d-flex align-items-center gap-2">
                    <i class="bi bi-type-h1 text-primary"></i> ชื่อบทความ
                </label>
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') }}" placeholder="กรอกชื่อบทความที่น่าสนใจ (ไม่เกิน 50 ตัวอักษร)" autofocus>
                @error('title')
            ...
            <div class="mb-4">
                <label for="content" class="form-label fw-bold text-dark d-flex align-items-center gap-2">
                    <i class="bi bi-card-text text-primary"></i> เนื้อหาบทความ
                </label>
                <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="7" placeholder="เขียนรายละเอียดเนื้อหาบทความของคุณที่นี่...">{{ old('content') }}</textarea>
                @error('content')
                    <div class="invalid-feedback d-flex align-items-center gap-1 mt-2">
                        <i class="bi bi-exclamation-circle-fill"></i> {{ $message }}
                    </div>
            ...
            <div class="mb-4">
                <label for="status" class="form-label fw-bold text-dark d-flex align-items-center gap-2">
                    <i class="bi bi-toggle-on text-primary"></i> สถานะการเผยแพร่
                </label>
                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                    <option value="1" {{ old('status', '1') == '1' ? 'selected' : '' }}>🟢 เผยแพร่ทันที (Active)</option>
                    <option value="0" {{ old('status') == '0' ? 'selected' : '' }}>🟡 บันทึกเป็นแบบร่าง (Draft)</option>
                </select>
                @error('status')
                    <div class="invalid-feedback d-flex align-items-center gap-1 mt-2">
                        <i class="bi bi-exclamation-circle-fill"></i> {{ $message }}
            ...
            <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center gap-3">
                ...
                <button type="submit" class="btn btn-gradient rounded-pill px-5 py-2 w-100 w-sm-auto fw-bold">
                    <i class="bi bi-cloud-arrow-up-fill me-1"></i> บันทึกบทความ
                </button>
            </div>
        </form>
    </div>
